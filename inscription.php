						...
						if(!preg_match("#^[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?@[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?\.[a-z]{2,30}$#i",$_POST['mail'])){
							...
							if(strlen($_POST['mail'])<7 or strlen($_POST['mail'])>50){
								...
								if(!$mysqli) {
									...
									if(mysqli_num_rows(mysqli_query($mysqli,"SELECT * FROM users WHERE pseudo='$Pseudo'"))!=0){//si mysqli_num_rows retourne pas 0
										...
										if(mysqli_query($mysqli,"INSERT INTO users SET pseudo='$Pseudo', mdp='$Mdp', mail='$Mail'")){
